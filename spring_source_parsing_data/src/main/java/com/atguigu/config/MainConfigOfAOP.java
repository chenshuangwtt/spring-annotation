 ...
 * 2. 定义一个逻辑类（MathCalculator） 在业务逻辑运行的时候将日志进行打印（方法之前，方法运行结束，方法出现异常，xxx）
 ...
 *   @Import(AspectJAutoProxyRegistrar.class)    给容器导入AspectJAutoProxyRegistrar注册器
 ...
 *          5. 注册没有实现优先级接口的 BeanPostProcessor  即普通的BeanPostProcessor
 ...
 *         2. 创建bean
 *          【AnnotationAwareAspectJAutoProxyCreator 在所有bean创建之前有个拦截，InstantiationAwareBeanPostProcessor会调用postProcessBeforeInstantiation()】
 *            1. 先从缓存中获取当前bean，如果能获取到，说明bean是之前被创建好的, 直接使用，否则再创建
 *                 只要创建好的Bean都会被缓存下来
 *            2. createBean()  创建bean
 *              AnnotationAwareAspectJAutoProxyCreator会在任何bean创建之前先尝试返回bean的实例
 *            【BeanPostProcessor是在Bean对象创建完成初始化前后调用的】
 *            【InstantiationAwareBeanPostProcessor 是在创建bean实例之前 先尝试用后置处理返回对象的】
 ...
 *       1. 判断当前bean是否保存在advisedBeans中【保存了所有需要增强的bean】
 ...
 *              1. 找到候选的所有增强器（找哪些通知方法是需要切入到当前bean方法的）
 *              2. 找到能在当前bean使用的增强器
 *              3. 给增强器排序
 *         2.保存当前bean到 advisedBeans中
 ...
 *
 *      3. 如果没有拦截器链方法，直接执行目标方法
 *         拦截器链（每一个通知方法又被包装为方法拦截器 利用MethodInterceptor机制）
 *      4. 如果有拦截器链，把需要执行的方法，目标方法，拦截器链等信息传入创建一个CglibMethodInvocation对象
 *        并调用Object retVal = mi.proceed();
 *      5. 拦截器链的触发过程
 *         1. 如果没有拦截器直接执行目标方法 或者拦截器的索引和拦截器数组-1大小一样（指定到了最后一个拦截器）执行目标方法
 *         2. 链式获取每个拦截器，拦截器执行invoke方法，每一个拦截器等待下一个拦截器执行完成返回以后再执行
 ...
 *             3. 效果:
 *                正常执行:  前置通知  -> 目标方法-> 后置方法 -> 返回通知
 *                出现异常:  前置通知  -> 目标方法-> 后置方法 -> 异常通知
 *
 */
@EnableAspectJAutoProxy
@Configuration
