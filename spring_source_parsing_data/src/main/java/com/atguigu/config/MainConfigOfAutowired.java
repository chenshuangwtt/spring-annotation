...
/**
 * 自动装配
 *   Spring利用依赖注入(DI) 完成对IOC容器中各个组价的依赖关系赋值
 *
 * 1.AutoWired 自动注入
 ...
 *          BookDao bookDao;
 *      }
 * 2. Spring还是使用@Resource(JSR250)和@Inject(JSR330)[java规范的注解]
 *      @Resource
 *          可以和AutoWired一样实现自动装配功能，默认是按组件名称进行装配的
 ...
 *   3.1 [标注在方法位置] @Bean+方法参数 参数从容器中获取, 默认不写@Autowired效果都是一样的，都能自动装配
...
@Configuration
@ComponentScan({"com.atguigu.controller","com.atguigu.dao",
        "com.atguigu.service","com.atguigu.bean"})
