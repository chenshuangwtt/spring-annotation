 ...
 *      在BeanFactory标准初始化之后调用，所有的bean定义保存加载到beanFactory，但bean的实例还未创建
 *
 ...
 *
 *     步骤:
 *       1. 写一个监听器(ApplicationListener实现类)来监听事件，（ApplicationEvent及其子类）
 *          @EventListener注解
 *          原理: 使用EventListenerMethodProcessor处理器来解析方法上的@EventListener
 *       2. 把监听器加入到容器中
 *       3. 只要容器中有相关事件的发布，我们就能监听到这个事件；
 *          ContextRefreshedEvent  容器刷新完成（所有bean都创建完成）会发布这个事件
 *          ContextClosedEvent  关闭容器的事件
 *       4. 发布一个事件
 ...
 *                  2. 否则，同步的方式直接执行listenter方法: invokeListener(listener, event);
 *                      拿到listener回调onApplicationEvent方法
 *
 *         【事件多播器】派发器
 *          1. 容器创建对象 refresh()
 *          2. initApplicationEventMulticaster() 初始化ApplicationEventMulticaster；
 *              1. 先去容器中有没有id= applicationEventMulticaster的组件
